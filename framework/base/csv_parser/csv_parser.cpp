...
bool csv_parser::init(const char * input_file)
{
	...
	input_filename = reinterpret_cast<char*>(calloc(1, filename_length + 1));
	...
	snprintf(input_filename, filename_length + 1, "%s", input_file);
...
void csv_parser::_skip_lines(void)
{
	...
	unsigned int number_of_lines_to_ignore = abs(static_cast<int>(ignore_num_lines));
...
void csv_parser::_get_fields_without_enclosure(csv_row_ptr row, const char * line, const unsigned int * line_length)
{
	...
	if (*line_length > 0)
	{
		field = reinterpret_cast<char*>(malloc(*line_length));
...
void csv_parser::_read_single_line(char ** buffer, unsigned int * buffer_len)
{
	...
	if (length_of_row > 0)
	{
		*buffer_len = length_of_row + 1;

		*buffer = reinterpret_cast<char*>(realloc(*buffer, *buffer_len));
