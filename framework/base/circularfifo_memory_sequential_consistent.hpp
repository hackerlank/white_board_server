...
#ifndef NDEBUG
        std::thread::id* _push_tid;
        std::thread::id* _pop_tid;
#endif // NDEBUG
    };
...
#ifndef NDEBUG
        if (!_push_tid) {
            _push_tid = new std::thread::id();
            *_push_tid = std::this_thread::get_id();
        } else {
            // allow only one producer, one consumer.
            assert(*_push_tid == std::this_thread::get_id());
        }
#endif // NDEBUG
...
#ifndef NDEBUG
        if (!_pop_tid) {
            _pop_tid = new std::thread::id();
            *_pop_tid = std::this_thread::get_id();
        } else {
            // allow only one producer, one consumer.
            assert(*_pop_tid == std::this_thread::get_id());
        }
#endif // NDEBUG
