...
#ifndef FRAMEWORK_BASE_LUA_TINKER_HPP_
#define FRAMEWORK_BASE_LUA_TINKER_HPP_
...
struct lua_lstring
{
	...
	lua_lstring(char *d, size_t l) : data(d), len(l) {}
	lua_lstring(uint8_t *d, size_t l) : data((char*)d), len(l) {}
};
...
template<>  void push(lua_State *L, lua_lstring *ret);
...
{
  (static_cast<user*>(lua_touserdata(L, 1)))->~user();
  return 0;
}
...
struct table
{
  ...
  table(const struct table &itable);
//  table(lua_State* L);
...
#endif  // FRAMEWORK_BASE_LUA_TINKER_HPP_
