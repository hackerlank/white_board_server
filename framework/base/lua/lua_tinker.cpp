...
{
	char temp[64];
	snprintf(temp, sizeof(temp),"%" I64_FMT "d", *(long long*)lua_topointer(L, 1));
...
{
	char temp[64];
	snprintf(temp, sizeof(temp), "%" I64_FMT "u", *(unsigned long long*)lua_topointer(L, 1));
	lua_pushstring(L, temp);
	return 1;
}
...
void lua_tinker::print_error(lua_State *L, const char* fmt, ...)
{
	...
	{
		printf("%s\n", text);
        LOG(ERROR) << "[lua_thinker  error] " << text;
		lua_pop(L, 1);
	}
}
...
long lua_tinker::read(lua_State *L, int index)
{
	if(lua_isnumber(L,index))
		...
		return *reinterpret_cast<long*>(lua_touserdata(L, index));
}
...
{
	*reinterpret_cast<long *>(lua_newuserdata(L, sizeof(long))) = ret;
...
template<>
void lua_tinker::push(lua_State *L, lua_tinker::lua_lstring *ret)
{
...
lua_tinker::table::table(const struct table &itable) {
	//TODO(jszhang): to be test in the future
	m_obj = new table_obj(nullptr, 0);
	memcpy(m_obj, itable.m_obj, sizeof(*m_obj));
}
